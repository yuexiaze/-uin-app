					...
					<view class="search-result">
						...
						<view class="search-result-item" v-for="(item,index) in searchList" :key="index" @tap="handleToDetail(item.id)">
							<view class="search-result-word">
								...
								<view>{{ item.name }}</view>
								<view><image v-if="item.fee==1" src="../../static/VIP.png" mode=""></image>{{ item.artists[0].name }} - {{ item.album.name }}</view>
							</view>
							<text class="iconfont iconbofang"></text>
						</view>
					</view>
...
<style scoped>
	...
	.search-result-word{ flex:1;text-overflow:ellipsis;white-space:nowrap;overflow: hidden;}
	.search-result-word image{height: 21rpx; width: 32rpx;}
